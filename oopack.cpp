...
// RESTYPE norm
// SOURCE http://www.kai.com/oopack/oopack.html
// SOURCE original file name: oopack_v1p8.C
// CATEGORY c++
// END
...
//
void Benchmark::time_both() const {
	// Run the C-style code.
	double r1, r2;

	/*	TIME_CYCLES (c_style ());   r1 = GET_CYCLES;
	TIME_CYCLES (oop_style ()); r2 = GET_CYCLES;
	RESULT (r2/r1);
	*/
	c_style();
	oop_style();
...
#define BENCH(name)	\
{	\
	Benchmark * b = Benchmark::find(name);	\
	if( b==NULL ) {	\
		ERROR ("non-existant subtest " name);	\
	} else {	\
		TAG(name);	\
		b->time_both();	\
	}	\
